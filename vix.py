...
def getHistDayOptions(var, date):
    ...
    optionsInfo = DataAPI.OptGet(optID=optionsID, varSecID='510050.XSHG',contractStatus = [u"DE", u"L"], field=fieldNeeded, pandas="1")
    optionsInfo = optionsInfo.set_index(u"optID")
    # optionsInfo=optionsInfo[optionsInfo['varSecID']=='510050.XSHG']
    # print var, date
    # return optionsInfo, optionsMkt
...
def getStrikeMinCallMinusPutClosePrice(options):
    ...
    return strike, abs(callMinusPut).min()
...
def calSigmaSquare(options, FF, R, T):
    ...
    # 去除同一行权价
    callAll = callAll[~callAll.index.duplicated()]
    putAll = putAll[~putAll.index.duplicated()]
...
def update():
    his_vix=pd.read_pickle('vix/vix.pkl')
    last_date=his_vix.index[-1]
    # today = Date.todaysDate()
    yesterday=Date.todaysDate()-Period('1d')
    data=getHistVIX(last_date, yesterday)
    his_vix.combine_first(data).to_pickle('vix/vix.pkl')
    # return his_vix.combine_first(data)
def today_vix():
    today = Date.todaysDate() 
    return getDayVIX(today)
def plot_vix(update_flag=False):
    if update_flag:
        update()
    plt.figure(figsize=(30,10))
    his_vix=pd.read_pickle('vix/vix.pkl')
    plt.plot(his_vix['vix'])
    plt.legend()
    plt.show()